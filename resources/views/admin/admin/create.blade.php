...
@section('content')
    ...
    <!-- 表单 -->
    <div class="col-md-7">
        <form action="{{ route('admins.store') }}" method="post" enctype="multipart/form-data" id="ajaxForm">
            {{csrf_field()}}
            ...
            <div>
                ...
                <div style="max-width: 600px; max-height: 400px">
                    <img src="{{ asset('uploads/avatars/default/default.jpg') }}" id="target">
                </div>
                ...
                <div class="form-group">
                    <div class="input-group">
                        ...
                        <label class="custom-file-label" for="selectNewFile">选择图片文件，或使用默认头像</label>
                    </div>
                </div>
            ...
            <button type="button" id="submit" class="btn btn-primary">新建</button>
        </form>
    </div>
...
@section('js')
    ...
    <script>
            ...
            $("#submit").on("click", function () {
                ...
                if ($('.custom-file-label').html() != new String('选择图片文件，或使用默认头像')) {
                    lengthArr.forEach((v) => {
                        let canvas = cropImage(v[1]);
                        let base64url = canvas.toDataURL('image/jpeg');
